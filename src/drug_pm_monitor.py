...
# ==============================
# 5️⃣ GESTION DE L’HISTORIQUE
# ==============================
if HISTORY_FILE.exists():
    df_history = pd.read_csv(HISTORY_FILE, parse_dates=["pm_update_date", "detected_on"])
else:
    df_history = pd.DataFrame(
        columns=["drug_code", "pm_update_date", "detected_on"]
    )
...
merged = df_current.merge(
    df_history.sort_values("detected_on")
